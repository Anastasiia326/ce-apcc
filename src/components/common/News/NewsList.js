    ...
    <div className="container mx-auto mb-[120px]">
      {
        NewsConfig.map((item, index) =>
          <NewBlog key={index} title={item.title} imgPath={item.imgPath}>
            <p className="text-[#333333] text-2xl font-normal leading-[40px] mb-16">A group of funders led by Wellcome have commissioned, through an open competitive process, an African-led team to design an ambitious initiative.
            ...
            <Link to="#" className="relative flex flex-row items-center w-fit z-10 text-[#1BAE70] hover:text-[#333333] text-2xl xl:text-3xl font-normal leading-[40px]">
      ...
      <div className="text-center"><button className="text-[#1BAE70] text-3xl font-normal leading-[40px] decoration-2 decoration-[#1BAE70] underline underline-offset-8">Load more</button></div>
    </div>
  )
}
